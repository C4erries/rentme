...
- [x] 15) Адекватизация ML-цены: нормализация города + клампы + пересборка датасетов (RUB, `rate_rub`, `ML_PRICE_CLAMPS`, `scripts/validate_ml_pricing.ps1`)

---

## Дополнительные правки (после "рубли везде")

- [x] A) ML: добавить `way=transit` (общественный транспорт) помимо `walk`/`car`
  - Обновить контракт ML: `way` поддерживает `walk|car|transit`.
  - Backend заполняет `travel_mode=transit` (мастер объявления/seed) и корректно прокидывает в ML.
  - Быстрый смоук: `POST /predict` с `way=transit` не падает, выдаёт разумную цену.

- [x] B) Просмотр объявления: модалка/окно карточки не помещается в экран
  - Исправить layout: модалка должна помещаться по высоте viewport, со скроллом внутри, без "обрезанных" CTA.
  - Проверка минимум на 360px и 1280px ширине.

- [x] C) Каталог: пометка "Ваше объявление" для хоста
  - Если пользователь с ролью `host` видит в каталоге своё объявление - показать бейдж "Ваше объявление".
  - UX-деталь: для своего объявления скрыть/задизейблить действия типа "Написать арендодателю".

- [x] D) ML: "слегка адекватизировать" влияние ремонта на цену
  - Проблема: сейчас при росте `renovation_score` цена иногда падает (анти-интуитивно).
  - Цель для демо: на 2-3 эталонных профилях при прочих равных повышение `renovation_score` не снижает прогноз.
  - Разрешено: править CSV-датасеты, генерацию данных, гиперпараметры/модель в пределах `scikit-learn` (без новых сервисов).

- [x] E) Бронирование long_term: корректная цена и UX дат
  - Сейчас long_term считается как "цена за месяц * кол-во дней" - это неверно.
  - Требования:
    - `long_term`: убрать дату выезда из UX (вместо этого: start date + длительность в месяцах, либо start date + "до отмены" в демо).
    - Итоговая стоимость: `rate_rub` (за месяц) * `months` (или фиксированный период по правилам демо), без умножения на дни.
    - Backward-compat: `short_term` сценарий не ломается.

- [x] F) Подтверждение брони хостом: UI/навигация
  - Сейчас после брони статус "ожидает подтверждение", но у хоста нет понятного места где увидеть и подтвердить.
  - Требования:
    - Добавить экран/раздел у хоста: входящие запросы на бронь (pending).
    - Кнопки "Подтвердить" / "Отклонить", обновление статуса и уведомление в UI.
    - Смоук: гость создаёт бронь -> хост подтверждает -> статус виден обоим.

- [x] G) "Мои бронирования": кнопка перейти в чат для каждой брони
  - Для гостя и хоста: на карточке брони есть кнопка "Перейти в чат".
  - Переход открывает чат с контрагентом (host<->guest) в контексте соответствующей брони/объявления.

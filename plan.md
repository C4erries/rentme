...
Дорожная карта по интеграции ML-прайсинга и выводу проекта в стабильный учебный маркетплейс аренды жилья (краткосрочная и долгосрочная аренда).
...
- [x] 7) Уведомления: оповещения о новых сообщениях (простая реализация, без Kafka)
- [x] 8) Админка: список пользователей + админ-чат с пользователями + метрики ML-моделей
- [x] 9) Docker Compose: все сервисы через compose, убрать Kafka/лишнее, добавить messaging-service + Scylla
- [x] 10) Полировка: валидации, ошибки/логи, документация, базовые тесты/скрипты
...
- DTO/типы фронта и бэка приведены к единому набору полей (этаж/этажность/ремонт/возраст дома/площадь и т.д.).
...
- Датасеты разделены: `mlrent/clean_train_long.csv` + `mlrent/clean_test_long.csv`; `mlrent/clean_train_short.csv` + `mlrent/clean_test_short.csv`.
- Добавлен сидер демо-данных: несколько арендодателей (агентства/частники) и ~30 short_term + ~30 long_term объявлений.
...
### 5) Лендинг + отзывы (сделано)
- Рабочие ссылки «Как это работает» и «Истории гостей».
- Отзывы end-to-end: создание отзыва гостем, вывод отзывов у объявления, агрегирование рейтинга в `Listing.Rating`.

### 6) Мессенджер (gRPC + Scylla) (сделано)
- Отдельный сервис `messaging-service`, хранит историю в ScyllaDB.
- Основной backend (Mongo) общается с мессенджером по gRPC.
- У каждого объявления есть `HostID`, чтобы гость мог написать арендодателю.

### 7) Уведомления (сделано)
- Минимальная система оповещений о новых сообщениях без Kafka (polling/WebSocket — по месту).

### 8) Админка (сделано)
- Список пользователей и встроенный админ-чат (выбор пользователя и переписка).
- Метрики ML-моделей (качество на test + базовые stats).

### 9) Docker Compose (сделано)
- Все сервисы поднимаются через compose: backend, frontend, ML, Mongo, MinIO, Scylla, messaging-service.
- Kafka и связанный код убраны, так как событийная архитектура не используется.

### 10) Полировка (сделано)
- Тексты, логи и UX доведены до стабильного состояния; документация обновлена, сборки проверены.

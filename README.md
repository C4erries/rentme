...
- Тарифы: для `short_term` цена указывается за ночь, для `long_term` — за месяц. **Единица денег в проекте фиксируется как рубли**; в API используем `rate_rub` + `price_unit` (`night`/`month`).

...
func (h *SubmitReviewHandler) Handle(ctx context.Context, cmd SubmitReviewCommand) (dto.Review, error) {
	...
	if err := h.recalculateListingRating(ctx, unit, booking.ListingID, now); err != nil {
		return dto.Review{}, err
	}
...
func (h *SubmitReviewHandler) recalculateListingRating(ctx context.Context, unit uow.UnitOfWork, listingID domainlistings.ListingID, now time.Time) error {
